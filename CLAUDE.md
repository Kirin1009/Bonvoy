...
| 担当 | 役割 |
|---|---|
| オーナー（Shun） | 意思決定: スコープ・テイストの最終判断・マージ承認 |
| **Claude（自分）** | **PM＋実装**: 進行管理、`docs/要件定義.md` の維持、index.html本体の実装、ホテルデータ投入、バージョン管理・リリース |
| Codex | デザイナー: 素材制作・モック発展・CSS/演出提案・デザインレビュー（ルールは `AGENTS.md`） |

- 分業の境界: **Codexは `design/`・`assets/` に納品、index.html等の本体統合はClaudeが行う**（本体の同時編集による競合を防ぐ）
- Claudeは進行上の判断で迷ったら（スコープ変更・テイスト・お金がかかる話・データ削除系）オーナーに聞く。それ以外は進める
- デザイン統合フロー: Codexの納品PR → Claudeが本体へ統合 → 390px確認 → PR → オーナー承認でマージ
